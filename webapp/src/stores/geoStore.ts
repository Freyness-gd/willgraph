...
export const useGeoStore = defineStore("geoStore", {
  ...
  actions: {
    ...
    toggleRoute(poi: PoiWithDistance) {
      if (poi.transportPath) {
        poi.transportPath = null;
      } else {
        void this.calculateTransportPath(poi)
      }
    },
    ...
    async calculateTransportPath(poi: PoiWithDistance) {
      ...
      if (poi.isLoadingRoute) return;

      poi.isLoadingRoute = true;
      const estateLat = this.selectedEstate.address.location.latitude;
      const estateLon = this.selectedEstate.address.location.longitude;
      ...
      try {
        const response = await poiService.getTransportPath(estateLat, estateLon, poi.lat, poi.lon);
        poi.transportPath = response;
      } catch (error) {
        console.error("Failed to load route", error);
      } finally {
        poi.isLoadingRoute = false;
      }
    },
